...
log() {
  ...
  if [ -f "$LOG" ]; then
    ...
    if [ "$lines" -ge "$MAX_LOG_LINES" ]; then
      echo "[$ts] 日志超过 $MAX_LOG_LINES 行，清空重新记录" > "$LOG"
    fi
  fi
  echo "[$ts] $*" >> "$LOG"
}
...
while true; do
  ...
  if [ -z "$NET_TABLE" ]; then
    log "没有找到 NET_TABLE (没有匹配的 lookup rmnet_data*)，稍后重试"
    sleep "$SLEEP_INTERVAL"
    continue
  fi
  ...
  for IFACE in $(list_rmnet_ifaces); do
    ...
    if check_iface_connectivity "$IFACE"; then
      ...
      # 优化：一旦找到一个可用接口并切换成功，可以选择跳出内层循环（避免频繁切换）；
      # 这里保留继续检查其余接口的行为（若需要可在此处加 'break'）
    else
      log "接口 $IFACE 无法访问外网，跳过"
    fi
  done
